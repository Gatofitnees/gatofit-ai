    ...
    <div className="bg-card border border-border rounded-xl overflow-hidden mb-4">
      <div className="p-4">
        <div className="flex justify-between items-start">
          ...
          <span className="bg-secondary/20 text-xs px-2 py-1 rounded-full text-secondary-foreground">
          ...
          {setsArray.map((set, setIndex) => (
            <div key={setIndex} className="flex justify-between items-center text-sm p-2 bg-secondary/10 rounded-lg">
              <div className="flex items-center">
                <span className="font-medium mr-2 text-card-foreground">Serie {set.set_number}</span>
                <span className="text-card-foreground">{set.reps_min}-{set.reps_max} reps</span>
              </div>
